...
double HairStrand::g_structuralKs = 5000.000; //3k 
double HairStrand::g_structuralKd = 5.000; //10
double HairStrand::g_shearKs = 4000.000; //4000
double HairStrand::g_shearKd = 6.00; //10
double HairStrand::g_bendKs = 4000.0000; //3000
double HairStrand::g_bendKd = 5.00; // 7
double HairStrand::g_penaltyKs = 6000.000; //5000
double HairStrand::g_penaltyKd = 320.000; //10

double HairStrand::friction_Mk = 0.5;
double HairStrand::friction_Amp = 2;
double HairStrand::COLLISION_THRESHOLD = 0.01; 
double HairStrand::jelloStartY = 1.3; //0.0

// TODO
double HairStrand::g_attachmentKs = 0.000;
double HairStrand::g_attachmentKd = 0.000;

HairStrand::HairStrand() :     
    m_integrationType(HairStrand::RK4), m_drawflags(MESH | STRUCTURAL),
...
void HairStrand::Reset()
{
    InitHairStrand();
}

HairStrand::Particle& HairStrand::GetParticle(HairStrand::ParticleGrid& grid, int i, int j, int k)
{
    return grid[i][j][k];
}

HairStrand::Particle& HairStrand::GetParticle(HairStrand::ParticleGrid& grid, int idx)
{
    int i,j,k;
    GetCell(idx, i, j, k);
...
const HairStrand::Particle& HairStrand::GetParticle(const HairStrand::ParticleGrid& grid, int i, int j, int k) const
{
    return grid[i][j][k];
}

const HairStrand::Particle& HairStrand::GetParticle(const HairStrand::ParticleGrid& grid, int idx) const
{
    int i,j,k;
    GetCell(idx, i, j, k);
    return GetParticle(grid, i,j,k);
...
bool HairStrand::isInterior(const HairStrand::Spring& s) const
{
...
bool HairStrand::isInterior(int idx) const
{
    int i,j,k;
    GetCell(idx, i, j, k);
...
bool HairStrand::isInterior(int i, int j, int k) const
{
    return (i*j*k*(m_rows-i)*(m_cols-j)*(m_stacks-k) != 0);
}

void HairStrand::SetGridSize(int cols, int rows, int stacks)
{
    m_cols = cols;
    m_rows = rows;
    m_stacks = stacks;
...
int HairStrand::GetGridCols() const
{
    return m_cols;
}

int HairStrand::GetGridRows() const
{
    return m_rows;
}

int HairStrand::GetGridStacks() const
{
    return m_stacks;
}

void HairStrand::SetSize(float width, float height, float depth)
{
    m_width = width;
    m_height = height;
    m_depth = depth;
...
float HairStrand::GetWidth() const
{
    return m_width;
}

float HairStrand::GetHeight() const
{
    return m_height;
}

float HairStrand::GetDepth() const
{
    return m_depth;
}

int HairStrand::GetIndex(int i, int j, int k) const
{
    int cols = j;
    int rows = i*(m_cols+1);
...
void HairStrand::GetCell(int idx, int& i, int &j, int& k) const
{
    float rows = m_rows+1;
    float cols = m_cols+1;
...
void HairStrand::InitHairStrand()
{
    m_vsprings.clear();
...
void HairStrand::AddStructuralSpring(Particle& p1, Particle& p2)
{
...
void HairStrand::AddBendSpring(HairStrand::Particle& p1, HairStrand::Particle& p2)
{
    double restLen = (p1.position - p2.position).Length();
    m_vsprings.push_back(Spring(BEND, p1.index, p2.index, g_bendKs, g_bendKd, restLen));
}

void HairStrand::AddShearSpring(HairStrand::Particle& p1, HairStrand::Particle& p2)
{
    double restLen = (p1.position - p2.position).Length();
    m_vsprings.push_back(Spring(SHEAR, p1.index, p2.index, g_shearKs, g_shearKd, restLen));
}

void HairStrand::SetIntegrationType(HairStrand::IntegrationType type)
{
    m_integrationType = type;
}

HairStrand::IntegrationType HairStrand::GetIntegrationType() const
{
    return m_integrationType;
}

void HairStrand::SetDrawFlags(unsigned int flags)
{
    m_drawflags = flags;
}

unsigned int HairStrand::GetDrawFlags() const
{
    return m_drawflags;
}

void HairStrand::DrawMesh(const vec3& eyePos)
{
...
void HairStrand::DrawSprings(double a)
{
    const ParticleGrid& g = m_vparticles;
    glBegin(GL_LINES);
...
void HairStrand::DrawCollisionNormals()
{
    const ParticleGrid& g = m_vparticles;
    glBegin(GL_LINES);
...
void HairStrand::Draw(const vec3& eyePos)
{
...
void HairStrand::Update(double dt, const World& world, const vec3& externalForces)
{
...
void HairStrand::CheckForCollisions(ParticleGrid& grid, const World& world)
{
    m_vcontacts.clear();
    m_vcollisions.clear();
...
void HairStrand::ComputeForces(ParticleGrid& grid)
{
...
void HairStrand::ResolveContacts(ParticleGrid& grid)
{
...
void HairStrand::ResolveCollisions(ParticleGrid& grid)
{
...
bool HairStrand::FloorIntersection(Particle& p, Intersection& intersection)
{
...
bool HairStrand::SphereIntersection(Particle& p, World::Sphere* sphere, Intersection& intersection) {
...
bool HairStrand::CylinderIntersection(Particle& pt, World::Cylinder* cylinder, 
                                 HairStrand::Intersection& intersection)
{
...
void HairStrand::EulerIntegrate(double dt)
{
...
void HairStrand::MidPointIntegrate(double dt)
{
...
void HairStrand::RK4Integrate(double dt)
{
	double halfdt = 0.5 * dt;
	ParticleGrid target = m_vparticles; // target is a copy!
	ParticleGrid& source = m_vparticles; // source is a ptr!
...
// Spring
//---------------------------------------------------------------------
HairStrand::Spring::Spring() :
    m_type(HairStrand::STRUCTURAL), 
    m_p1(-1), 
    m_p2(-1), 
...
HairStrand::Spring::Spring(const HairStrand::Spring& p) :
    m_type(p.m_type), m_p1(p.m_p1), m_p2(p.m_p2),
    m_Ks(p.m_Ks), m_Kd(p.m_Kd), m_restLen(p.m_restLen)
{
}

HairStrand::Spring& HairStrand::Spring::operator=(const HairStrand::Spring& p)
{
...
HairStrand::Spring::Spring(HairStrand::SpringType t, 
...
HairStrand::Particle HairStrand::Particle::EMPTY;

HairStrand::Particle::Particle(int idx, const vec3& p, const vec3& v, double m)
{
    index = idx;
    position = p;
    velocity = v;
...
HairStrand::Particle::Particle() : index(-1), position(0,0,0), velocity(0,0,0), force(0,0,0), mass(1.0)
{
}

HairStrand::Particle::Particle(const HairStrand::Particle& p) : 
    index(p.index), position(p.position), velocity(p.velocity), force(p.force), mass(p.mass)
{
}

HairStrand::Particle& HairStrand::Particle::operator=(const HairStrand::Particle& p)
{
    if (&p == this) return *this;
...
HairStrand::Intersection::Intersection(const HairStrand::Intersection& p) :
	m_p(p.m_p), m_normal(p.m_normal), m_distance(p.m_distance), m_type(p.m_type), m_ground_friction(p.m_ground_friction)
{
}

HairStrand::Intersection& HairStrand::Intersection::operator=(const HairStrand::Intersection& p)
{
    if (&p == this) return *this;
    m_p = p.m_p;
    m_normal = p.m_normal;
...
HairStrand::Intersection::Intersection(IntersectionType type, int p, const vec3& normal, double d) :
...
void HairStrand::FaceMesh::Draw(const HairStrand& m)
{
...
void HairStrand::FaceMesh::DrawNormals(const HairStrand& m)
{
    glDisable(GL_LIGHTING);
...
HairStrand::FaceMesh::FaceMesh(const HairStrand& m, HairStrand::Face f)
{
    const ParticleGrid& g = m.m_vparticles;
    switch(f)
    {
...
void HairStrand::FaceMesh::CalcDistToEye(const HairStrand& m, const vec3& eyePos)
{
...
bool HairStrand::FaceMesh::compare(const FaceMesh& one, const FaceMesh& other)
{
